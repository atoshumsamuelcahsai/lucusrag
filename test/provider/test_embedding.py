...
def test_get_embeddings_raises_when_api_key_missing(monkeypatch):
    # Register a dummy provider that requires an API key
    monkeypatch.setattr(factory_mod, "_EMBEDDING_REGISTRY", {}, raising=True)
    ...
    def make_dummy():
        ...
        api_key = os.getenv("DUMMY_API_KEY")
        if not api_key:
            raise EnvironmentError("DUMMY_API_KEY not found")
        return type("DummyEmb", (), {"get_text_embedding": lambda self, text: [0.0]})()

    with pytest.raises(EnvironmentError) as e:
        factory_mod.get_embeddings("dummy")
    assert "DUMMY_API_KEY" in str(e.value)
...
def test_get_embeddings_calls_factory_with_api_key(monkeypatch):
    ...
    # Capture the api_key used in the factory
...
def test_default_provider_resolves_voyage_when_registered(monkeypatch):
    ...
    @factory_mod.register_embedding("voyage")
    def make_voyage():
        import os
