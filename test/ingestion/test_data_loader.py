...
def mock_db_manager():
    ...
    manager.driver.return_value = AsyncMock()
    manager.driver.return_value.session = AsyncMock(return_value=mock_session)
    manager.driver.return_value.session.return_value.__aenter__ = AsyncMock(
        return_value=mock_session
    )
    manager.driver.return_value.session.return_value.__aexit__ = AsyncMock(
        return_value=False
    )
    # All methods are now async
    manager.create_schema = AsyncMock()
    manager.create_node = AsyncMock()
    manager.create_relationships = AsyncMock()
...
class TestProcessASTFiles:
    ...
    @pytest.mark.asyncio
    async def test_process_ast_files_empty_directory(self, tmp_path):
        """Test processing an empty directory."""
        documents, code_infos = await _process_ast_files(tmp_path)
    ...
    async def test_process_ast_files_with_valid_json(
    ...
    @pytest.mark.asyncio
    @patch("rag.ingestion.data_loader.process_code_element")
    async def test_process_ast_files_multiple_files(self, mock_process, tmp_path):
    ...
    @pytest.mark.asyncio
    @patch("rag.ingestion.data_loader.process_code_element")
    async def test_process_ast_files_with_invalid_json(self, mock_process, tmp_path):
    ...
    @pytest.mark.asyncio
    @patch("rag.ingestion.data_loader.process_code_element")
    async def test_process_ast_files_sets_defaults(self, mock_process, tmp_path):
...
class TestCreateSchema:
    ...
    @pytest.mark.asyncio
    async def test_create_schema_success(self, mock_db_manager, vector_config):
        """Test successful schema creation."""
        await _create_schema(mock_db_manager, vector_config)

        mock_db_manager.create_schema.assert_called_once_with(vector_config)

    @pytest.mark.asyncio
    async def test_create_schema_failure(self, mock_db_manager, vector_config):
...
class TestBuildCodeDBGraph:
    ...
    @pytest.mark.asyncio
    async def test_build_code_db_graph_empty_list(self, mock_db_manager, vector_config):
        """Test building database graph with empty list."""
        await _build_code_db_graph(mock_db_manager, [], vector_config)
    ...
    @pytest.mark.asyncio
    async def test_build_code_db_graph_with_code_elements(
        ...
        await _build_code_db_graph(mock_db_manager, code_infos, vector_config)

        mock_db_manager.create_schema.assert_called_once()
        # With asyncio.gather, we need to check if it was called
        assert mock_db_manager.create_node.called
        assert mock_db_manager.create_relationships.called
    ...
    async def test_build_code_db_graph_continues_on_node_error(
        ...
        # Should attempt both (with asyncio.gather and return_exceptions=True)
        assert mock_db_manager.create_node.call_count == 2
        # Should still try to create relationships for both
        assert mock_db_manager.create_relationships.call_count == 2

    @pytest.mark.asyncio
    async def test_build_code_db_graph_continues_on_relationship_error(
        self, mock_db_manager, vector_config, sample_code_element
    ):
...
class TestCheckDBBuilt:
    ...
    @pytest.mark.asyncio
    async def test_check_db_built_true(self, mock_db_manager, vector_config):
        ...
        mock_db_manager.driver = AsyncMock(return_value=mock_driver)

        result = await _check_db_populated(mock_db_manager, vector_config)

        assert result is True
        mock_session.run.assert_called_once()

    @pytest.mark.asyncio
    async def test_check_db_built_false(self, mock_db_manager, vector_config):
    ...
    @pytest.mark.asyncio
    async def test_check_db_built_error(self, mock_db_manager, vector_config):
        ...
        with pytest.raises(
            ...
            await _check_db_populated(mock_db_manager, vector_config)
...
class TestProcessCodeFiles:
    ...
    @pytest.mark.asyncio
    async def test_process_code_files_directory_not_exists(self):
        """Test error when directory doesn't exist."""
        with pytest.raises(FileNotFoundError, match="does not exist"):
            await process_code_files("/nonexistent/path")

    @pytest.mark.asyncio
    async def test_process_code_files_path_is_file(self, tmp_path):
        """Test error when path is a file, not a directory."""
        test_file = tmp_path / "test.txt"
        test_file.write_text("test")

        with pytest.raises(FileNotFoundError, match="is not a directory"):
            await process_code_files(str(test_file))
    ...
    async def test_process_code_files_db_empty(
        self,
        mock_get_config,
        mock_db_class,
        ...
        result = await process_code_files(str(tmp_path))
    ...
    @patch("rag.ingestion.data_loader._check_db_populated", new_callable=AsyncMock)
    @patch("rag.ingestion.data_loader._process_ast_files")
    @patch("rag.ingestion.data_loader._build_code_db_graph")
    @patch("rag.ingestion.data_loader.populate_embeddings")
    @patch("rag.ingestion.data_loader.GraphDBManager")
    ...
    async def test_process_code_files_db_built(
        ...
        mock_db_manager.close = AsyncMock()
        mock_db_class.return_value = mock_db_manager
        # Make _check_db_populated return True (it's now an AsyncMock)
    ...
    @patch("rag.ingestion.data_loader._check_db_populated")
    @patch("rag.ingestion.data_loader._process_ast_files")
    async def test_process_code_files_with_dependency_injection(
        self,
        mock_process_files,
        mock_check_db,
        tmp_path,
        ...
        result = await process_code_files(
    ...
    @patch("rag.ingestion.data_loader.GraphDBManager")
    @patch("rag.ingestion.data_loader.get_vector_index_config")
    async def test_process_code_files_closes_manager_on_error(
        self,
        mock_get_config,
        mock_db_class,
        mock_process_files,
        ...
        with pytest.raises(Exception, match="Database error"):
            await process_code_files(str(tmp_path))
    ...
    @patch("rag.ingestion.data_loader.GraphDBManager")
    @patch("rag.ingestion.data_loader.get_vector_index_config")
    async def test_process_code_files_returns_empty_on_no_files(
        self,
        mock_get_config,
        mock_db_class,
        mock_process_files,
...
class TestProcessCodeFilesIntegration:
    ...
    @patch("rag.ingestion.data_loader.GraphDBManager")
    @patch("rag.ingestion.data_loader.get_vector_index_config")
    async def test_full_workflow_new_database(
        self,
        mock_get_config,
        mock_db_class,
        mock_check_db,
        ...
        mock_get_config.return_value = vector_config
        mock_db_manager = MagicMock()
        mock_db_manager.create_schema = AsyncMock()
        mock_db_manager.create_node = AsyncMock()
        mock_db_manager.create_relationships = AsyncMock()
        mock_db_manager.close = AsyncMock()
        mock_db_class.return_value = mock_db_manager
        ...
        assert len(result) == 1  # Returns list of documents
        mock_db_manager.create_schema.assert_called_once()
        # With asyncio.gather, these will be called
        assert mock_db_manager.create_node.called
        assert mock_db_manager.create_relationships.called
        mock_populate_embeddings.assert_called_once()
        mock_db_manager.close.assert_called_once()
